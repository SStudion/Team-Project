...
/** Submits a draft — validates required fields first per PRD 4.2.3, then flips draft → submitted. */
export async function submitApplication(applicationId) {
  ...
  // Validation happens before the status flip, so an incomplete application
  // can never reach an admin's queue.
  const { valid, errors } = validateApplicationForSubmit(app);
  if (!valid) {
    const err = new Error("Please complete all required fields before submitting.");
    err.fieldErrors = errors; // the form uses this to highlight what's missing
    throw err;
  }
