...
/**
 * Uploads, or in dummy mode just registers, a document for an application.
 * PRD requires size/format validation before accepting.
 *
 * Allowed in two states: DRAFT (the original 4-step form) and
 * MISSING_DOCUMENTS (Sprint 3 — client decision D-03/D-04: a student asked
 * for a missing document may add just that document, without the rest of
 * the application becoming editable again). Every other status stays closed
 * to uploads, matching "submitted applications remain non-editable".
 *
 * ⚠️ Frontend-only change: firestore.rules' `documents` create rule still
 * hard-codes `application.status == "draft"` (see firestore.rules, ~line
 * 137) — until that's updated too, an upload attempted while the
 * application is MISSING_DOCUMENTS will pass this check but still be
 * rejected by Firestore with a permission error. That rule change is
 * Kristina's side of this feature.
 */
export async function registerDocument({ applicationId, fileType, file }) {
  ...
  const uploadableStatuses = [STATUS.DRAFT, STATUS.MISSING_DOCUMENTS];
  if (!uploadableStatuses.includes(app.status)) {
    throw new Error(
      "Documents can only be added while the application is a draft, or while the university has requested missing documents."
    );
  }
