    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Card className="p-0 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e2e8f0]">
          ...
          <Link href="/admin-applications" className="text-xs text-[#2a5298] hover:underline flex items-center gap-1 shrink-0">
        ...
        <div className="md:hidden divide-y divide-[#e2e8f0]">
          {loading ? (
            <p className="text-center py-10 text-sm text-[#64748b]">Loading applications...</p>
          ) : recent.length === 0 ? (
            <p className="text-center py-10 text-sm text-[#64748b]">No applications yet.</p>
          ) : (
            recent.map((app) => (
              <div key={app.applicationId ?? app.id} className="px-4 py-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  <p className="text-xs text-[#64748b]">Updated {formatDate(app.updatedAt)}</p>
                  <Link
                    href={`/admin-applications/${app.applicationId ?? app.id}`}
